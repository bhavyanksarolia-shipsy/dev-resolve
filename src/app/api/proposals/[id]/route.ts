...
export async function POST(req: Request, ctx: RouteContext<"/api/proposals/[id]">) {
  ...
  if (action === "accept") {
    ...
    try {
      applyToFiles(target, file ?? p.file, content ?? p.content, `proposal #${p.id} · investigation #${p.investigation_id} · accepted ${new Date().toISOString().slice(0, 10)}`);
    } catch (e) {
      return Response.json({ error: (e as Error).message }, { status: 400 });
    }
  }
  await q(`UPDATE knowledge_proposals SET status=$2, content=COALESCE($3, content), file=COALESCE($4, file), decided_at=now() WHERE id=$1`, [
    id, action === "accept" ? "accepted" : "rejected", content ?? null, file ?? null,
  ]);
